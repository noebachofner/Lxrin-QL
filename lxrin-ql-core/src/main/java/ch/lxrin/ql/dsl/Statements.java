...
/**
 * The static methods shared by the entry points {@code ch.lxrin.ql.QL} and {@link Dsl}:
 * statements ({@code select}, {@code selectFrom}, {@code insertInto}, …), the
 * {@code createContribution} style, CTEs and windows. The condition operators
 * ({@link Conditions}), the function catalog ({@link Functions}), bind parameters and
 * literals ({@link Values}) and the typed {@code select(..)} overloads are inherited. Use
 * it through one of the entry points, not directly.
 *
 * <p>Statements created here run on {@link QueryContext#getDefault()}.</p>
 */
public class Statements extends Functions {

    /** Static members only; extended by {@link Dsl} and {@code ch.lxrin.ql.QL}. */
    ...
    /**
     * The field itself, for select and returning lists written in the style of 2.x:
     * {@code c.select(col(USERS.USERNAME))} is {@code c.select(USERS.USERNAME)} and keeps the
     * field's type.
     *
     * @throws IllegalArgumentException if {@code field} is {@code null}
     */
    public static <T> Field<T> col(Field<T> field) {
        if (field == null) throw new IllegalArgumentException("col(..) needs a field");
        return field;
    }
