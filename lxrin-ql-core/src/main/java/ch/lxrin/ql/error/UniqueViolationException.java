package ch.lxrin.ql.error;

import ch.lxrin.ql.schema.Constraint;

import java.sql.SQLException;

/** A unique constraint (or primary key) was violated: SQLSTATE {@code 23505}. */
public class UniqueViolationException extends ConstraintViolationException {

    private static final long serialVersionUID = 1L;

    /** See {@link ConstraintViolationException#ConstraintViolationException}. */
    public UniqueViolationException(String message, String sql, SQLException cause, String constraintName, String tableName,
            Constraint constraint) {
        super(message, sql, cause, constraintName, tableName, constraint);
    }
}
